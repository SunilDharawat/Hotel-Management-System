...
import { 
  sampleUsers, sampleCustomers, sampleRooms, sampleBookings, 
  sampleInvoices, sampleMessages, sampleTemplates, sampleSettings 
...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        dispatch({ type: 'LOAD_DATA', payload: { ...initialState, ...parsed, user: null } });
      } catch (error) {
  ...
  }, [state.customers, state.rooms, state.bookings, state.invoices, state.messages, state.templates, state.users, state.settings]);
  ...
  const hasPermission = (permission) => {
    ...
    const permissions = {
      admin: ['all'],
      manager: ['view_reports', 'manage_rooms', 'view_bookings', 'view_customers', 'view_billing', 'view_messages'],
      receptionist: ['manage_customers', 'manage_bookings', 'manage_checkin', 'manage_billing', 'send_messages']
    };
  ...
  return (
    <AppContext.Provider value={{ state, dispatch, login, logout, hasPermission }}>
      {children}
    </AppContext.Provider>
  );
}
...
export function useBookings() {
  ...
  return {
    ...
    updateBooking: (booking) => dispatch({ type: 'UPDATE_BOOKING', payload: booking })
  };
}
...
export function useInvoices() {
  ...
  return {
    ...
    addInvoice: (invoice) => dispatch({ type: 'ADD_INVOICE', payload: invoice })
  };
}
