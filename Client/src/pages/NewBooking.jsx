...
//   const isSameState = gstState.state_id === formData.customer_state_code;
...
//                               customer_id: value,
//                               customer_state_code:
//                                 selectedCustomer?.state_id || null,
//                             });
//                           }}
//                           disabled={createBookingMutation.isPending}
//                         >
...
export default function NewBooking() {
  ...
  const customers = customersData?.data?.customers || [];
  const availableRooms = availableRoomsData?.data?.rooms || [];
  const gstRates = settings?.gst_rates || { cgst: 6, sgst: 6, igst: 12 };
  const gstState = settings?.hotel_state || {};

  // Determine if same state or different state
  const isSameState = String(gstState.state_id) === formData.customer_state_id;
  ...
  const cgst = isSameState ? roomCharges * (gstRates.cgst / 100) : 0;
  const sgst = isSameState ? roomCharges * (gstRates.sgst / 100) : 0;
  const igst = !isSameState ? roomCharges * (gstRates.igst / 100) : 0;

  // Calculate total
  const totalTax = isSameState ? cgst + sgst : igst;
  const totalAmount = roomCharges + totalTax;
                        ...
                        <Select
                          ...
                          onValueChange={(value) => {
                            ...
                            setFormData({
                              ...
                              customer_id: value,
                              customer_state_id:
                                selectedCustomer?.customer_gst_state_code ||
                                null,
                            });
                          }}
                          disabled={createBookingMutation.isPending}
                    ...
                    {selectedRoom ? (
                      <>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          {/* GST Section - Conditional based on state */}
                          {isSameState ? (
                            <>
                              <div className="flex justify-between text-muted-foreground">
                                <span>CGST ({gstRates.cgst}%)</span>
