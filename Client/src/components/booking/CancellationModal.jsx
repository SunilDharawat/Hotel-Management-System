  ...
  const handleCancellation = () => {
    if (isEarlyCheckout) {
      ...
      const invoice = {
        ...
        items: [
          {
            description: `Room Charges - ${room?.type || 'Room'} (${nightsStayed} night${nightsStayed > 1 ? 's' : ''})`,
        ...
        notes: `Early checkout. Original checkout: ${format(originalCheckOut, 'MMM d, yyyy')}. Reason: ${reason}`,
        createdAt: new Date(),
        dueDate: new Date()
      };
      addInvoice(invoice);
    ...
    if (room && (room.status === 'occupied' || room.status === 'reserved')) {
      updateRoom({ ...room, status: isEarlyCheckout ? 'cleaning' : 'available' });
    }

    toast.success(isEarlyCheckout ? 'Early checkout processed successfully!' : 'Booking cancelled successfully!');
    onClose();
  };
      ...
      <DialogContent className="max-w-md bg-background">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-destructive">
            {isEarlyCheckout ? (
              <><AlertTriangle className="h-5 w-5" />Early Checkout</>
            ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="border-t pt-4 space-y-2 text-sm">
            {isEarlyCheckout ? (
              <>
                <div className="flex justify-between">
                  <span>Nights Stayed ({nightsStayed} × {formatCurrency(roomRate)})</span>
          ...
          {/* Reason */}
          <div>
            <Label>Reason for {isEarlyCheckout ? 'Early Checkout' : 'Cancellation'}</Label>
            <Textarea
              value={reason}
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button 
              ...
              className="flex-1"
            >
              {isEarlyCheckout ? 'Process Early Checkout' : 'Confirm Cancellation'}
            </Button>
          </div>
        </div>
      </DialogContent>
