...
export function CheckoutModal({ isOpen, onClose, booking }) {
  ...
  const isEarlyCheckout = today < originalCheckOut;

  // Calculate room charges based on actual stay
  const checkInDate = new Date(booking.check_in_date);
  const checkOutDate = isEarlyCheckout ? today : originalCheckOut;
  ...
  const handleCheckout = async () => {
    try {
      ...
      toast.success(
        isEarlyCheckout
          ? "Early checkout completed successfully!"
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {isEarlyCheckout ? (
              <>
                ...
                Early Checkout - Room {booking.room_number}
              </>
            ) : (
              <>
                <Receipt className="h-5 w-5" />
          ...
          {isEarlyCheckout && (
            <DialogDescription className="text-orange-600 font-medium">
              Guest is checking out before scheduled date. Charges calculated
              for actual stay.
            </DialogDescription>
          )}
        </DialogHeader>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">
                  {isEarlyCheckout ? "Scheduled Check-out" : "Check-out"}
                </span>
                <span>{format(originalCheckOut, "MMM d, yyyy")}</span>
              </div>
              {isEarlyCheckout && (
                <div className="flex justify-between font-medium text-orange-600">
                  <span>Actual Check-out (Today)</span>
                  <span>{format(today, "MMM d, yyyy")}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-muted-foreground">
                  {isEarlyCheckout ? "Nights Stayed" : "Duration"}
                </span>
                <span>
                  {nights} night{nights > 1 ? "s" : ""}
                </span>
