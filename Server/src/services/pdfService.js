...
//         doc.text(`Email: ${invoice.customer_email || "N/A"}`, 40, doc.y + 2);
//         doc.text(
//           `Address: ${invoice.booking.customer_state_name || "N/A"} [${invoice.booking.customer_state_code || "N/A"}]`,
//           40,
//           doc.y + 2,
//         );
  ...
  static async generateInvoicePDF(invoice, settings = {}) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // CONDITIONAL GST DISPLAY LOGIC
        const hasCGST = invoice.cgst_rate && parseFloat(invoice.cgst_rate) > 0;
        const hasSGST = invoice.sgst_rate && parseFloat(invoice.sgst_rate) > 0;
        const hasIGST = invoice.igst_rate && parseFloat(invoice.igst_rate) > 0;
        ...
        if (hasCGST && hasSGST) {
          ...
          addTotalLine(
            ...
            `${parseFloat(invoice.igst_amount).toFixed(2)}`,
          );
          addTotalLine(
            `Total GST (${invoice.igst_rate}%):`,
            `${parseFloat(invoice.igst_amount).toFixed(2)}`,
          );
        }
