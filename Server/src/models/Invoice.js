...
class Invoice {
  ...
  //       `SELECT b.*, r.room_number, r.type as room_type, c.id as customer_id, c.state_id
  //        FROM bookings b
    ...
    try {
      ...
      const [bookings] = await connection.execute(
        `SELECT b.*, r.room_number, r.type as room_type, 
              c.id as customer_id, c.state_id
       FROM bookings b
       JOIN rooms r ON b.room_id = r.id
       JOIN customers c ON b.customer_id = c.id
       WHERE b.id = ?`,
      ...
      // Get hotel state from settings
      const hotelStateSetting = await Setting.getByKey("hotel_state");
      const hotelState = hotelStateSetting.setting_value;
      console.log(hotelStateSetting, "h");

      // Get customer state
      const customerState = booking.state_id
        ? await State.findById(booking.state_id)
        : null;
      console.log(booking.state_id, "b");
      console.log(customerState, "c");

      // Get GST rates from settings
      const gstRatesSetting = await Setting.getByKey("gst_rates");
      const gstRates = gstRatesSetting.setting_value;
      console.log(gstRatesSetting, "g");
      ...
      const gstData = calculateGSTWithState(
        subtotal,
        hotelState,
        customerState,
        gstRates,
      ...
      // Create invoice with state-based GST
      const invoiceId = uuidv4();
      const invoiceNumber = generateInvoiceNumber();
