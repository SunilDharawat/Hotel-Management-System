...
/**
 * Verify JWT token and attach user to request
 * - Header token for all APIs
 * - Query token ONLY for PDF routes
 */
const authenticate = async (req, res, next) => {
  try {
    let token = null;
    ...
    // 2️⃣ Query token (ONLY for PDF routes)
    else if (isPdfRoute && req.query?.token) {
...
/**
 * Optional authentication (no changes required)
 */
const optionalAuth = async (req, res, next) => {
  try {
    const authHeader = req.headers.authorization;
